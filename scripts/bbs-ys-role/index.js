/*
 * @Author: kasuie
 * @Date: 2024-08-04 13:12:06
 * @LastEditors: kasuie
 * @LastEditTime: 2024-08-04 17:26:58
 * @Description:
 */
(function () {
  "use strict";
        ...
        if (res.message === "OK") {
          ...
          if (list && list[0]) {
            ...
            const roleRes = request({
              method: "POST",
              url: `https://localhost:8001/ys/updateRoles?queryId=true`,
            ...
            const weaponRes = request({
              method: "POST",
              url: `http://localhost:8001/ys/updateWeapons?queryId=true`,
              headers: { "Content-Type": "application/json" },
              data: JSON.stringify(weaponsVo),
            });

            Promise.all([roleRes, weaponRes]).then(([res1, res2]) => {
